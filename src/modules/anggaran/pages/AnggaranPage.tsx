    ...
    return (
        <div className="space-y-6 max-w-[1600px] mx-auto pb-10 animate-fade-slide">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Manajemen Anggaran</h2>
                    <p className="text-sm text-gray-500">Monitoring plafon dan realisasi rencana anggaran tahunan paroki.</p>
                </div>
                {isBendahara && (
                    <Button
                        onClick={() => setIsModalOpen(true)}
                        className="flex items-center gap-1.5 text-xs bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 shadow-sm"
                    >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="p-4 border-l-4 border-l-indigo-500 border-y-slate-200 border-r-slate-200 shadow-sm">
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Total Plafon Anggaran</p>
                    <h4 className="text-lg font-black mt-1 text-slate-800 tracking-tight">{formatIDR(metrics.totalPlafon)}</h4>
                ...
                <Card className="p-4 border-l-4 border-l-rose-500 border-y-slate-200 border-r-slate-200 shadow-sm">
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Realisasi (Terpakai)</p>
                    <h4 className="text-lg font-black mt-1 text-rose-600 tracking-tight">{formatIDR(metrics.totalRealisasi)}</h4>
                    <p className="text-[9px] text-rose-600 mt-2 font-bold flex items-center gap-1">
                ...
                <Card className="p-4 border-l-4 border-l-emerald-500 border-y-slate-200 border-r-slate-200 shadow-sm">
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Sisa Anggaran Tersedia</p>
                    <h4 className="text-lg font-black mt-1 text-emerald-600 tracking-tight">{formatIDR(metrics.totalSisa)}</h4>
                    <p className="text-[9px] text-emerald-600 mt-2 font-bold">✓ Siap dialokasikan</p>
                </Card>

                <Card className="p-4 border-l-4 border-l-amber-500 border-y-slate-200 border-r-slate-200 shadow-sm">
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Persentase Penyerapan</p>
                    <div className="flex items-end gap-1 mt-1">
                        <h4 className="text-lg font-black text-slate-800 tracking-tight">{Math.round(metrics.burnRate)}%</h4>
                ...
                <Card className="lg:col-span-4 p-5 border-slate-200 shadow-sm flex flex-col justify-between">
                    ...
                    <div className="h-[180px] relative">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Sisa Total</span>
                            <span className="text-sm font-black text-emerald-600">{formatIDR(metrics.totalSisa, { notation: 'compact' })}</span>
                        </div>
                    </div>
                        ...
                        <AdaptiveList
                            ...
                            renderDesktopRow={(item) => (
                                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-5 py-2.5 text-xs font-bold text-slate-500 border-r border-slate-100 text-center">{item.tahun}</td>
                                    ...
                                    <td className={cn("px-5 py-2.5 text-xs text-right font-black text-emerald-600", isBendahara && "border-r border-slate-100")}>{formatIDR(item.sisa)}</td>
                                    {isBendahara && (
                                        <td className="px-5 py-2.5 text-center">
                                            <Button
                                                ...
                                                variant="outline"
                                                className="text-[10px] px-2 py-0.5 font-bold h-6 border-blue-200 text-blue-600 hover:bg-blue-50/50"
                                            >
                                                Edit
                                            </Button>
                                        </td>
                            ...
                            renderMobileCard={(item) => (
                                <div className="flex flex-col gap-2.5">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-500">{item.tahun} - {Math.round(item.percentUsed)}% Terpakai</span>
                                    </div>
                                    ...
                                    {isBendahara && (
                                        <div className="pt-2 border-t border-slate-100 flex justify-end">
                                            <Button
                                                ...
                                                variant="outline"
                                                className="text-[9px] px-2 py-0.5 font-bold h-6 border-blue-200 text-blue-600 hover:bg-blue-50/50"
                                            >
                                                Edit Anggaran
                                            </Button>
                                        </div>
